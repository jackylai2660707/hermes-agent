...
def check_image_generation_requirements() -> bool:
    ...
    1. The in-tree OpenAI-compatible backend when explicitly configured.
    2. The in-tree FAL backend (FAL_KEY or managed gateway).
    3. Any plugin-registered provider whose ``is_available()`` returns True.

    Plugins win only when the in-tree paths are NOT ready, which matches
