...
def _restart_should_use_service_manager() -> bool:
    """Return True when the current gateway process is service-managed."""
    ...
    try:
        return os.getpid() in _get_service_pids()
    except Exception:
        return False
...
class GatewayRunner:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> str:
        ...
        active_agents = self._running_agent_count()
        # Persist the requester so the restarted gateway can notify them
