...
def _build_embedded_profile_env(config: dict[str, Any], *, llm_api_key: str | None = None) -> dict[str, str]:
    ...
    if idle_timeout is None or idle_timeout == "":
        idle_timeout = os.environ.get("HINDSIGHT_EMBED_DAEMON_IDLE_TIMEOUT", "3600")
...
def _embedded_profile_env_path(config: dict[str, Any]):
    ...
    if hindsight_home:
        profile_root = Path(hindsight_home)
    elif os.getuid() == 0 and Path.home() == Path("/root"):
        try:
            import pwd

            profile_root = Path(pwd.getpwnam("hermes").pw_dir) / ".hindsight"
        except KeyError:
            profile_root = Path.home() / ".hindsight"
    else:
        profile_root = Path.home() / ".hindsight"
    return profile_root / "profiles" / f"{_embedded_profile_name(config)}.env"
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if self._mode == "local_embedded":
            if globals().get("HindsightEmbedded") is None:
                available, reason = _check_local_runtime()
                if not available:
                    logger.warning(
